...
class UCF101SummaryDataset(Dataset):
    """UCF101 dataset for video summarization
    ...
    Args:
        ...
        # interval (int): number of frames between selected frames.
        # tuple_len (int): number of selected frames in each tuple, 3/4/5.
        summary_len (int): number of key frames (elements of summary)
        transforms_ (object): composed transforms which takes in PIL image and output tensors.
    """
    def __init__(self, root_dir, summary_len, train=True, transforms_=None):
        self.root_dir = root_dir
    ...
    def __getitem__(self, idx):
        """
        Returns:
            tuple_key_frame (tensor): [summary_len x channel x height x width]
        """
        if self.train:
            videoname = self.train_split[idx]
        else:
        ...
        # get summarization( key frame list)
        # 1) naive - random k frame
        
        # random select frame for train, deterministic random select for test
        if self.train:
            tuple_key_frame_idx_arr = random.sample(range(length), self.summary_len)
        else:
            random.seed(idx)
            tuple_key_frame_idx_arr = random.sample(range(length), self.summary_len)
        tuple_key_frame_idx_arr = sorted(tuple_key_frame_idx_arr)
        
        tuple_key_frame = [videodata[frame_idx] for frame_idx in tuple_key_frame_idx_arr]
        
        # TODO 2) summarization network output
        
        if self.transforms_:
            trans_tuple = []
            for frame in tuple_key_frame:
                frame = self.toPIL(frame) # PIL image
                frame = self.transforms_(frame) # tensor [C x H x W]
                trans_tuple.append(frame)
            tuple_key_frame = trans_tuple
        else:
            tuple_key_frame = [torch.tensor(frame) for frame in tuple_key_frame]

        return torch.stack(tuple_key_frame)
